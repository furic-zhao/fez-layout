<template>
  <aside class="main-sidebar animated" :class="{ showSlide: sidebar.show, hideSlide: !sidebar.show, expandSide:(!sidebar.collapsed||device.isMobile)}">
    <fez-scrollbar tag="div" wrapClass="vue-scrollbar" v-if="(!sidebar.collapsed||device.isMobile)">
      <div class="sidebar">
        <fez-menu :default-active="onRoutes" :default-openeds="onRouteKeys" class="fezlay-menu-style" theme="light" router :collapse="sidebar.collapsed&&!device.isMobile" @select="handleSelect">
    ...
    <div class="sidebar" v-else>
      <fez-menu :default-active="onRoutes" class="fezlay-menu-style" theme="light" router :collapse="sidebar.collapsed&&!device.isMobile" @select="handleSelect">
        <template v-for="item in menuList">
          <sub-menu :param="item"></sub-menu>
        </template>
      </fez-menu>
